        ...
        for i = 1:length(params.signals)
            ...
            elseif length(params.signals{i}) > 6 && strcmpi(params.signals{i}(end-6:end),'neurons') % do raster
                ...
                if ~isempty(tc) % sort by PD and only include tuned cells
                    ...
                    is_tuned = all(prctile(tc(comp_blocks(1)).(use_model).rs,[2.5 97.5],2) > 0.5,2) & ...
                        all(prctile(tc(comp_blocks(2)).(use_model).rs,[2.5 97.5],2) > 0.5,2) & ...
                        all(prctile(tc(comp_blocks(3)).(use_model).rs,[2.5 97.5],2) > 0.5,2) & ...
                ...
                imagesc(t-params.dt/2,1:length(I),sd.(params.signals{i})(:,I)'); %colorbar;
