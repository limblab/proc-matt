...
root_dir = 'F:\TrialDataFiles\biomech_model\';
...
for iFile = 1:length(filenames)
    load([root_dir filenames{iFile} '_tuning.mat'],'tc_data','params');
    load([root_dir filenames{iFile} '_neurons.mat'],'neural_tcs');
...
for iBlock = 1:3
    ...
    for iFile = 1:length(filenames)
        load([root_dir filenames{iFile} '_tuning.mat'],'tc_data','neural_tcs','params');
...
if 1
    ...
    for iFile = 1:length(filenames)
        iFile
        load([root_dir filenames{iFile} '_tuning.mat'],'sw_data','tc_data');
