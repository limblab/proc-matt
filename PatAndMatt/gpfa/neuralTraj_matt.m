...
%
% directory   - string containing the directory you want to use to save
%                 results (eg directory = 'C://users/limblab/Desktop').
%                 Pass in an empty vector to skip saving.
%
% OUTPUTS:
%
...
fprintf('\n---------------------------------------\n');
if ~isempty(directory) % pass in empty to skip saving .mat
    if ~isdir([directory '\\mat_resultsgpfa'])
        mkdir([directory '\\mat_resultsgpfa']);
    end
    % Make a directory for this runIdx if it doesn't already exist
    runDir = sprintf([directory '\\mat_resultsgpfa\\run%s'], runIdx);
    %   runDir = fullfile(directory,'mat_resultsgpfa',['run' num2str(runIdx)]);
    if isdir(runDir)
...
for cvf = 0:numFolds
    ...
    result.cvf = cvf;
    result.hasSpikesBool = hasSpikesBool;
    result.extraOpts = extraOpts;
    
    if ~isempty(fname) && exist([fname '.mat'], 'file')
